...
keywordList(){
 ...
 multimediaList(artURL){
	if(artURL.split(".").pop() === "jpg")
	  {
		  return(
		  <img src = {artURL} alt="https://image.freepik.com/icones-gratis/ponto-de-interrogacao_318-52837.jpg"/>)  
	  }
      if(artURL.split(".").pop() === "mp3"){
			   return(
				 <audio controls>
                 <source src={artURL} type="audio/mpeg"/>
                 </audio>)
		  }
	  if(artURL.split(".").pop() === "mp4"){
		  return(
			   <video controls>
               <source src={artURL} type="video/mp4"/>
               </video>)
		  }
 } 
    ...
    return (
	 <div className="container-fluid">
	  ...
	  <Navbar staticTop >
        ...
        <center><h2>{this.state.piece.id.pieceName}</h2></center>
        <div className="row">
          <div className="well col-md-6 col-md-push-3">
		  
          <div className="multimedia"> {this.renderMultimedia()} </div>		  

		   <p> <b>Author:</b> {this.state.piece.id.authorName}</p>
		   <p> <b>Description:</b>{this.state.piece.description} </p>
			...
			<p> <b>Owner:</b> {this.state.piece.owner}</p>
